...
public class DeviceSettings extends AppCompatActivity {
    ...
    private void getTrustedDevices() {
        Cognito_Connection.getPool().getUser(Cognito_Connection.getCurrUser()).listDevicesInBackground(10, null, devicesHandler);
    }
    ...
    DevicesHandler devicesHandler = new DevicesHandler() {
        ...
        public void onSuccess(List<CognitoDevice> devices) {
            CognitoDevice thisDevice = Cognito_Connection.getPool().getUser(Cognito_Connection.getCurrUser()).thisDevice();
            Cognito_Connection.setThisDevice(thisDevice);
            if (devices == null) {
                // Show no trusted devices are available.
                List<CognitoDevice> emptyDevicesList = new ArrayList<CognitoDevice>();
                Cognito_Connection.setDevicesForDisplay(emptyDevicesList);
            } else {
                // Show the trusted devices on the screen.
                Cognito_Connection.setDevicesForDisplay(devices);
            }
            showTrustedDevices();
        }
        ...
        public void onFailure(Exception e) {
            showDialogMessage("Devices",Cognito_Connection.formatException(e),false);
        }
    };

    private void showTrustedDevices() {
        TextView currDeviceState = (TextView) findViewById(R.id.currDeviceState);
        CognitoDevice thisDevice = Cognito_Connection.getThisDevice();
        if (Cognito_Connection.getThisDeviceTrustState()) {
            if (thisDevice != null) {
        ...
        devicesListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                showDeviceDetail(Cognito_Connection.getDeviceDetail(position));
            }
        });
        closeWaitDialog();
    }
    ...
    private void changeDeviceTrustState(CognitoDevice device, boolean trustDevice) {
        if (device == null) {
            if (Cognito_Connection.getThisDevice() != null) {
                if (trustDevice) {
                    Cognito_Connection.getThisDevice().rememberThisDeviceInBackground(deviceStatusChange);
                } else {
                    Cognito_Connection.getThisDevice().doNotRememberThisDeviceInBackground(deviceStatusChange);
                }
            }
        } else {
            if (trustDevice) {
    ...
    GenericHandler deviceStatusChange = new GenericHandler() {
        ...
        public void onFailure(Exception exception) {
            showDialogMessage("Devices",Cognito_Connection.formatException(exception),false);
        }
    };
